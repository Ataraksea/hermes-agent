...
| `HERMES_GEMINI_CLI_COMMAND` | Override Gemini CLI binary path (default: `gemini`) |
...
| `GEMINI_CLI_BASE_URL` | Override Gemini CLI backend marker (default: `cli://gemini`) |
...
| Variable | Description |
|----------|-------------|
| `HERMES_INFERENCE_PROVIDER` | Override provider selection: `auto`, `openrouter`, `nous`, `openai-codex`, `copilot`, `copilot-acp`, `gemini-cli`, `anthropic`, `huggingface`, `zai`, `kimi-coding`, `minimax`, `minimax-cn`, `kilocode`, `xiaomi`, `alibaba`, `deepseek`, `opencode-zen`, `opencode-go`, `ai-gateway` (default: `auto`) |
